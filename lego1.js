...
let enforceTraining = true//for debug use false. when true disable the next button till user do instruction. also forced to false for test userId 666
...
function animateModelAboveButtons(model, onAnimationComplete = null, instant = false) {
    ...
    if (fiveDotBlock) {
        ...
        // Collect completed models separately in each world.
        // Models from another world must not affect this model's position.
        const modelWorld =
            currentSession &&
                currentSession.worldByModel
                ? currentSession.worldByModel[
                model.metadata.modelTitle
                ]
                : null;
